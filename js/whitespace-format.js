...
import { unbleach } from "./helpers/unbleach.js";
...
function whitespaceNumber(n) {
  ...
  for (let i = 0; i < binary.length; i++) {
    if (!isNaN(binary[i]))
      res += conversion[binary[i]];
  }

  return res + "\n";
}

Test.assert(unbleach(whitespaceNumber(1)), unbleach(" \t\n"));
Test.assert(unbleach(whitespaceNumber(0)), unbleach(" \n"));
Test.assert(unbleach(whitespaceNumber(-1)), unbleach("\t\t\n"));
Test.assert(unbleach(whitespaceNumber(2)), unbleach(" \t \n"));
Test.assert(unbleach(whitespaceNumber(-3)), unbleach("\t\t\t\n"));
